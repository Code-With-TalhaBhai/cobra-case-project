...
export default function Steps() {
    ...
    {STEPS.map((step,index)=>{
        const isCurrent : boolean = pathname.endsWith(step.url)
        const isCompleted : boolean = STEPS.slice(index+1).some((step)=>pathname.endsWith(step.url))
        const imgPath = `/snake-${index+1}.png`
        return <li key={index} className="relative overflow-hidden lg:flex-1">
            <div>
            ...
            {/* Separator */}
                    {
                        index !== 0 ? <div className="absolute inset-0 hidden w-3 lg:block">
                            <svg
