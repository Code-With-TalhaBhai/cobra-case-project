...
export default function Upload(){
    ...
    //     // Sending FormData with the File object
    //     const formData = new FormData()
    //     formData.append('file_key',configId)
    //     formData.append('file',file)
    //     formData.append('file_type',file.type)
    ...
    //     try {
    //       const fileUrl = await uploadFile_to_server(formData);
    //       // Assuming uploadFile_to_server handles progress or you don't need a separate progress bar
    //       // This example removes the XMLHttpRequest logic as server actions don't support it directly
          
    //       if (fileUrl?.status === 200 && fileUrl.url) {
    ...
    //               router.push(`/configure/design?id=${configuration.id}`);
    //             } catch (prismaError) {
    //               console.error('Prisma post error:', prismaError);
    //               toast({
    //                 title: 'Database error',
    //                 description: 'Could not save image configuration. Please try again.',
    //                 variant: 'destructive',
    //               });
    //               setIsUploading(false);
    //             }
